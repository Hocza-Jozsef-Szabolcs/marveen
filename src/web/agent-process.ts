...
export function resolveProviderEnv(
  ...
  // mixed-case (measured live 2026-08-19: the Settings UI Save wrote
  // "MiniMax-M3" instead of the canonical "minimax-m3"). A case-sensitive
  // `.startsWith` here silently misroutes a mixed-case minimax- id to the
  // Ollama branch (wrong ANTHROPIC_BASE_URL, and the CLAUDE_CODE_MAX_CONTEXT_TOKENS
  // override below never fires) -- contextLimitForModel in context-guard.ts
  // already lowercases first, so this discriminator was the one place that
  // didn't match it. `model` itself (original casing) still goes into the
  // ANTHROPIC_MODEL export below -- only the routing decision is case-insensitive.

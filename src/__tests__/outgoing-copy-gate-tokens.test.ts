...
// Ported unchanged from upstream (Szotasz/marveen), commits d9bb515 and
// 70a2be7. GATEKOTOJEL817 + GATEHYPH816: two false positives in five
// minutes, in a live owner conversation, both the same class -- the gate
// could not tell PROSE from IDENTIFIER. (1) `Drive-ot`: a Hungarian suffix
// attaches to a foreign proper noun WITH a hyphen (that is the correct
// spelling); the letters-only tokenizer cut at the hyphen and read the `ot`
// remainder as a standalone Hungarian word (ot -> öt). (2) `Video
// atalakitas`: a Drive FOLDER NAME quoted in prose -- a mid-sentence
// capitalized word is an identifier, not prose. The fix is TOKENIZATION,
// not the dictionary (a word exception list would also pass real errors):
// hyphenated forms are checked as the WHOLE token, and mid-sentence
// capitalized words are skipped -- while sentence-start capitals and
// lowercase prose remain fully checked.
...
// GATESZAMKOTOJEL821: the same prose-vs-identifier class, one step further.
// HYPHEN_WORD admits only LETTERS around the hyphen, so a Hungarian suffix
// attached to a NUMBER ("429-es", "2026-os") is tokenized as a bare word --
// and "es" then reads as the accent-stripped "és". A digit before the
// hyphen is the signal: that token is part of an identifier, not prose.

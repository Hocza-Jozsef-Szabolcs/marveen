...
// Ported unchanged from upstream (Szotasz/marveen), commits a480d68/5cbadd7
// (KAPUHATOKOR822) and fd9806f (RESENDGATE826). KAPUHATOKOR822: four false
// positives in one afternoon, on THREE operation types (inter-agent message,
// sqlite write, file READ). The old trigger searched the WHOLE command
// string for send-patterns, so the '"to":' of an inter-agent envelope plus
// 'send.py' mentioned in the CONTENT read as an email send -- the gate
// silenced the fleet on exactly the topic it most needs to talk about. The
// trigger now works on COMMAND POSITION: heredoc bodies and quoted strings
// are cut first, then the INVOKED program of each pipeline segment decides.
// These tests pin both directions: content can no longer fake a send, and
// every real send shape still fires.
...
describe('RESENDGATE826: the resend trigger fires on the METHOD, not the hostname', () => {
  ...
  it('KNOWN POSITIVE: a real POST /emails still fires, in every spelling', () => {
    expect(isSend(`curl -X POST https://api.resend.com/emails -H "Authorization: Bearer X" -d '{"to":"a@b.hu"}'`)).toBe(true)
    expect(isSend(`curl 'https://api.resend.com/emails' -d @/tmp/mail.json`)).toBe(true)
    expect(isSend(`curl -sX POST https://api.resend.com/emails -d '{}'`)).toBe(true)

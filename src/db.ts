...
export function initDatabase(dbPathOverride?: string): void {
  ...
  // Migration: project tag (which product line -- VrMobile, JokerQ, VHR5, ...
  // -- a fact belongs to). Distinct from agent_id (which fej owns the row)
  // and category (hot/warm/cold/shared tier). NULL for rows with no specific
  // product line (personal/cross-cutting facts).
  try {
...
export function saveAgentMemory(
  ...
  const info = db.prepare(
    'INSERT INTO memories (chat_id, topic_key, content, sector, salience, created_at, accessed_at, agent_id, category, auto_generated, keywords, project) VALUES (?, ?, ?, ?, 1.0, ?, ?, ?, ?, ?, ?, ?)'
  ).run(ALLOWED_CHAT_ID, null, content, 'semantic', now, now, agentId, category, autoGenerated ? 1 : 0, keywords ?? null, project ?? null)
